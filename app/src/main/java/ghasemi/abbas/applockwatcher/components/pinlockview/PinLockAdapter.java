package ghasemi.abbas.applockwatcher.components.pinlockview;
...
public class PinLockAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private static final int VIEW_TYPE_GO = 2;
    ...
    private void configureNumberButtonHolder(NumberViewHolder holder, int position) {
        if (holder != null) {
            if (position == 9) {
                if (isGo && mPinLength > 3) {
                    holder.mNumberButton.setVisibility(View.VISIBLE);
                    holder.mNumberButton.setText("تائید");
                    holder.mNumberButton.setTag(10);
                } else {
    ...
    public void setGoButton(boolean is) {
        isGo = is;
    }
    ...
    public class NumberViewHolder extends RecyclerView.ViewHolder {
        ...
        public NumberViewHolder(final View itemView) {
            ...
            mNumberButton.setTypeface(Typeface.createFromAsset(mNumberButton.getContext().getAssets(), "fonts/Main-Bold.ttf"), Typeface.BOLD);
            mNumberButton.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    int p = (Integer) v.getTag();
                    if (p == 10) {
                        if (isGo && mOnDeleteClickListener != null) {
                            mOnDeleteClickListener.onGoClicked();
                        }
                    } else if (mOnNumberClickListener != null) {
                        mOnNumberClickListener.onNumberClicked(p);
                    }
                }
            });
        }
                ...
                mDeleteButton.setOnTouchListener(new View.OnTouchListener() {
                    ...
                    public boolean onTouch(View v, MotionEvent event) {
                        ...
                        if (event.getAction() == MotionEvent.ACTION_UP) {
                            mButtonImage.setColorFilter(0xffe2e2e2);
                        }
                        if (event.getAction() == MotionEvent.ACTION_MOVE) {
                            if (!rect.contains(v.getLeft() + (int) event.getX(),
                                    v.getTop() + (int) event.getY())) {
                                mButtonImage.setColorFilter(0xffe9e9e9);
                            }
                        }
                        return false;
                    }
